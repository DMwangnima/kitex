...
func TestNewStream(t *testing.T) {
	...
	t.Run("success with StreamRecvTimeout", func(t *testing.T) {
		...
		// StreamRecvTimeout should be set at ttstream layer
		test.Assert(t, cliSt.recvTimeout == 100*time.Millisecond, cliSt.recvTimeout)
		test.Assert(t, cliSt.stream.recvTimeoutCallback != nil)
	})

	t.Run("success with StreamRecvTimeoutConfig, ttstream layer should NOT set recvTimeout", func(t *testing.T) {
		cconn, sconn := newTestConnectionPipe(t)
		defer cconn.Close()
		defer sconn.Close()
		...
		cliSt := cs.(*clientStream)
		// StreamRecvTimeoutConfig is handled by the outer layer, ttstream should NOT set recvTimeout
		test.Assert(t, cliSt.recvTimeout == 0, cliSt.recvTimeout)
		test.Assert(t, cliSt.stream.recvTimeoutCallback == nil)
	})
	...
	t.Run("both set, StreamRecvTimeoutConfig takes priority", func(t *testing.T) {
		...
		cliSt := cs.(*clientStream)
		// StreamRecvTimeoutConfig takes priority, ttstream should NOT set recvTimeout
		test.Assert(t, cliSt.recvTimeout == 0, cliSt.recvTimeout)
		test.Assert(t, cliSt.stream.recvTimeoutCallback == nil)
	})
