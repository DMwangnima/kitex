...
func Test_ttheaderStream_SendHeader(t *testing.T) {
	...
	// 缺少已经 SendHeader，再 SendHeader
	// 已经 SendMsg，再 SendHeader
	// 已经 close，再 SendHeader
}
...
func Test_ttheaderStream_SetTrailer(t *testing.T) {
	...
	// 为什么对于 SendHeader，已经 Send 过了或者 已经 Close 掉了，再 Send 会报 ErrIllegalWrite ?
	// 而 SendTrailer 不会呢？
	t.Run("server:closed", func(t *testing.T) {
		var st *ttheaderStream
		ri := mockRPCInfo()
