...
// TestHTTP2Client_Dump tests the Dump method
func TestHTTP2Client_Dump(t *testing.T) {
	ctx := context.Background()
	conn := setupMockConn("127.0.0.1:8080")
	opts := ConnectOptions{
		KeepaliveParams: ClientKeepalive{
	...
	dump := client.Dump()
	if dump == nil {
		t.Fatal("expected dump to return non-nil value")
	}

	transportDump, ok := dump.(clientTransportDump)
	if !ok {
		t.Fatalf("expected dump to be clientTransportDump, got %T", dump)
	}

	// Verify dump contents
	if transportDump.State != reachable {
		t.Errorf("expected state to be reachable, got %v", transportDump.State)
	}

	if len(transportDump.ActiveStreams) != 3 {
		t.Errorf("expected 3 active streams, got %d", len(transportDump.ActiveStreams))
	}

	if transportDump.LocalAddress == "" {
		t.Error("expected local address to be set")
	}

	if transportDump.MaxConcurrentStreams == 0 {
		t.Error("expected max concurrent streams to be set")
	}
}
...
// TestCloseStreamTask tests the closeStreamTask functionality
func TestCloseStreamTask(t *testing.T) {
	ctx := context.Background()
	conn := setupMockConn("127.0.0.1:8080")
	opts := ConnectOptions{
		KeepaliveParams: ClientKeepalive{
	...
	// Create streams with cancelable contexts
	cancelCtx1, cancel1 := context.WithCancel(ctx)
	...
	stream1, err := client.NewStream(cancelCtx1, callHdr1)
	if err != nil {
		t.Fatalf("failed to create stream1: %v", err)
	}
	...
	stream2, err := client.NewStream(cancelCtx2, callHdr2)
	if err != nil {
		t.Fatalf("failed to create stream2: %v", err)
	}
	...
	// Create task and run tick
	task := &closeStreamTask{t: client}
	task.Tick()

	// Wait for cleanup
	time.Sleep(100 * time.Millisecond)

	// Verify stream1 is closed but stream2 is not
	select {
	case <-stream1.done:
		// Expected
	default:
		t.Error("expected stream1 to be closed")
	}

	select {
	case <-stream2.done:
		t.Error("expected stream2 to still be open")
	default:
		// Expected
	}
